...
- [x] **Multi-modal Transportation:** Allow users to switch between Bicycle, Car, and Walking profiles. Travel time (and therefore interval timestamps) must recalibrate correctly for each mode.
- [ ] **Custom Draggable Routes:** Implement a custom polyline editor so users can click/drag the generated route to force it down a specific road using Google Maps.
